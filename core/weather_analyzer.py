...
class WeatherAnalyzer:
    ...
    def analyze_forecast(self, forecast_data: Dict) -> str:
        """
        АНАЛИЗИРУЕТ ПРОГНОЗ ПОГОДЫ НА НЕСКОЛЬКО ДНЕЙ
        Возвращает текстовые рекомендации на основе анализа прогноза
        
        Args:
            forecast_data: Данные прогноза от WeatherAPIClient
            
        Returns:
            str: Текстовые рекомендации
        """
        try:
            ...
            # Анализируем каждый день прогноза
            for day in forecast_data['days'][:3]:  # Берем ближайшие 3 дня
                # Создаем временный объект WeatherData для анализа дня
                daily_weather = WeatherData()
                daily_weather.temperature = day.get('temp_avg', 0)
                daily_weather.humidity = day.get('humidity_avg', 0)
                daily_weather.wind_speed = day.get('wind_max', 0)
                daily_weather.precipitation = day.get('rain_total', 0) + day.get('snow_total', 0)
                daily_weather.precipitation_probability = day.get('precipitation_prob', 0) * 100
                
                # Определяем основное состояние погоды
                conditions = day.get('conditions', [])
                if conditions:
                    main_condition = conditions[0].lower()
                    condition_map = {
                    ...
                    daily_weather.condition = condition_map.get(main_condition, WeatherCondition.CLEAR)
                
                # Анализируем день
                day_analysis = self.comprehensive_analysis(daily_weather)
                date = day.get('date', 'Unknown')
                
                # Проверяем подходит ли день для мойки
                if day_analysis.washing_decision == Decision.WASH_RECOMMENDED:
                    best_wash_days.append(date)
                
                # Собираем серьезные риски
                for risk in day_analysis.risks:
                    if risk['level'] in [RiskLevel.HIGH, RiskLevel.CRITICAL]:
                        risks.append(f"{date}: {risk['description']}")
            
            # Формируем итоговые рекомендации
            if best_wash_days:
                recommendations.append("🧼 *Лучшие дни для мойки:*")
                for wash_day in best_wash_days:
                    recommendations.append(f"• {wash_day}")
            else:
                recommendations.append("🧼 *В ближайшие 3 дня мойку лучше отложить*")
            
            # Добавляем риски
            if risks:
                recommendations.append("\n⚠️ *Ожидаются неблагоприятные условия:*")
                unique_risks = list(set(risks))[:3]  # Берем 3 уникальных риска
                for risk in unique_risks:
                    recommendations.append(f"• {risk}")
            
            # Добавляем общую сводку по температуре
            temps = [day.get('temp_avg', 0) for day in forecast_data['days'][:3]]
            if temps:
            ...
            print(f"❌ Ошибка анализа прогноза: {e}")
            return "❌ Произошла ошибка при анализе прогноза"
